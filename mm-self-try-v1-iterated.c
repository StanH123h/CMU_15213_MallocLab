...
/**
  ...
  valid  util   ops    secs     Kops  trace
   yes     0%  100000  0.010806  9254 ./traces/alaska.rep
 * yes    23%    4805  0.001431  3357 ./traces/amptjp.rep
 * yes    30%    4162  0.000355 11711 ./traces/bash.rep
 * yes    44%   57716  0.006001  9618 ./traces/boat.rep
 * yes    19%    5032  0.001499  3358 ./traces/cccp.rep
 * yes    34%   11991  0.001061 11299 ./traces/chrome.rep
 * yes     2%   20000  0.003589  5573 ./traces/coalesce-big.rep
   yes     0%   14400  0.004530  3179 ./traces/coalescing-bal.rep
   yes   100%      15  0.000003  4687 ./traces/corners.rep
 * yes    30%    5683  0.001704  3335 ./traces/cp-decl.rep
 u yes     1%      --        --    -- ./traces/exhaust.rep
 * yes    32%    8000  0.000693 11549 ./traces/firefox.rep
   yes    63%   99804  0.007080 14097 ./traces/firefox-reddit.rep
   yes    68%     118  0.000022  5413 ./traces/hostname.rep
 * yes    65%   19405  0.001961  9896 ./traces/login.rep
 * yes    25%     200  0.000023  8772 ./traces/lrucd.rep
   yes    75%     372  0.000044  8435 ./traces/ls.rep
   yes    94%      10  0.000002  5556 ./traces/malloc.rep
   yes    77%      17  0.000002 10000 ./traces/malloc-free.rep
 *  no      -       -         -     - ./traces/needle.rep
 * yes    34%     200  0.000031  6349 ./traces/nlydf.rep
   yes    71%    1494  0.000162  9245 ./traces/perl.rep
 * yes    32%     200  0.000025  7937 ./traces/qyqyc.rep
 * yes    36%    4800  0.001462  3284 ./traces/random.rep
 * yes    37%    4800  0.001523  3151 ./traces/random2.rep
 * yes    83%     147  0.000033  4441 ./traces/rm.rep
 * yes    28%     200  0.000024  8230 ./traces/rulsr.rep
 p yes     --    6495  0.000696  9335 ./traces/seglist.rep
   yes   100%      12  0.000004  3429 ./traces/short2.rep
            -         -     -
 */

/**
  第一版的迭代版的核心思路:
  和第一版一样:不释放，需要内存就分配新的，老的永远占用

  迭代内容: 解决了两个off-by-one问题
 */
...
{
    dbg_printf("========malloc()========\n");
    // 这里要+1是因为当时定义ALIGN_PTR的时候，其实是把参数ptr作为第一个可用的字节来看的
    char *new_mem_start = ALIGN_PTR((char *)mem_heap_hi() + 1);
    dbg_printf("1: mem_heap_hi=%p\n", mem_heap_hi());
    dbg_printf("2. new_mem_start=%p\n", new_mem_start);

    // 这里要-1是因为，我们这里的diff计算的是"从这里(开头不算)到下一个8的倍数的地址之间有几个无法使用的字节"
    // 所以比如下一个地址是8，当前是1，不能直接7-1，因为这把8这个位置也算进去了，8本身是可以使用的，所以还要-1
